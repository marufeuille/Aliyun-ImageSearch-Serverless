...
*/
const Multipart = {};
Multipart.parse = (body, contentType) => {
    ...
    if ( isRaw ) {
        // 2017/07/24 Change
        // Original Source Code causes stack overflow if BigFile input.
        const view = new Uint8Array( body );
        const buf = [];
        const len = 1024;
        for (var p = 0; p < view.byteLength; p += len) {
          buf.push(String.fromCharCode.apply("", new Uint16Array(view.slice(p, p + len))));
        }
        s = buf.join("");
